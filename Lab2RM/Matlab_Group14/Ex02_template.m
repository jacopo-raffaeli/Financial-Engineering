...
E_FV = sum(Q(1,:)*FV);   %  Present Value in a years time
...
bd =  norminv(Q(1,3)+Q(1,2));                % Barrier to down (HY)
...
scen_i = zeros(N,1);   % int.: Count aof "identical rating" events in each MC scenario
scen_u = zeros(N,1);   % int.: Count of "up" events in each MC scenario
for i = 1 : N_issuers
     V = rho.*Y+sqrt(1-(rho)^2).*randn(N,1);           % AVR of i-th issuer
...
ptf_DO = L_D.*scen_D;
ptf_DO_decr = sort(-ptf_DO, 'descend');
VaR_DO = ptf_DO_decr(floor(N*(1-alpha)));

% Default and migration case (DM)
ptf_DM = L_D.*scen_D + L_d.*scen_d + L_u.*scen_u + L_i.*scen_i;
ptf_DM_decr = sort(-ptf_DM, 'descend');
VaR_DM =ptf_DM_decr(floor(N*(1-alpha))) ;
