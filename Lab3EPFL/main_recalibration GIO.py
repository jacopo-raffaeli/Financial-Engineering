...
"""
def compute_pinball_loss(y_true, pred_quantiles, quantiles_levels):
  
    Utility function to compute the pinball loss on the test results
    return: pinball loss computed for each quantile level and each sample in the prediction horizon
    ...
    for i, q in enumerate(quantiles_levels):
        ...
        loss_q = np.maximum(q * error, (q - 1) * error)
        loss.append(loss_q)
    pinball_loss = np.sum(loss, axis=2) / len(quantiles_levels)
    return pinball_loss
"""
def compute_winkler_score(y_true, pred_quantiles, quantiles_levels):
    """
    Utility function to compute the Winkelr score on the test results
    return: winkler scores computed for each quantile level and each step in the pred horizon
    """
    score = []
    for i, q in enumerate(quantiles_levels):
        error = np.subtract(y_true, pred_quantiles[:, :, i])
        loss_q = np.abs(error) * (1 - np.abs(q - 0.5) * 2)
        score.append(np.expand_dims(loss_q, -1))
    score = np.mean(np.concatenate(score, axis=-1), axis=0)
    return score
#--------------------------------------------------------------------------------------------------------------------
...
#pinball_loss = compute_pinball_loss(y_true=test_predictions[PF_task_name].to_numpy().reshape(-1,pred_steps),
#                                        pred_quantiles=test_predictions.loc[:,test_predictions.columns != PF_task_name].
#                                        to_numpy().reshape(-1, pred_steps, len(quantiles_levels)),
#                                        quantiles_levels=quantiles_levels)
...
winkler_scores = compute_winkler_score(y_true=test_predictions[PF_task_name].to_numpy().reshape(-1,pred_steps),
                                       pred_quantiles=test_predictions.loc[:,test_predictions.columns != PF_task_name].
                                       to_numpy().reshape(-1, pred_steps, len(quantiles_levels)),
                                       quantiles_levels=quantiles_levels)
...
plot_quantiles(test_predictions, target=PF_task_name)
#--------------------------------------------------------------------------------------------------------------------
#media_colonne = np.mean(winkler_scores, axis=0)

APS = 1/21 * np.sum(pinball_scores, axis=1)

media = np.mean(APS)


# Definisci i bin per l'asse x (vettore da 0 a 100 con passo 5)
x_bins = np.arange(0, 24)

# Crea l'istogramma
plt.figure(figsize=(10, 6))  # Imposta la dimensione della figura
plt.bar(x_bins, APS, width=0.5, color='b', edgecolor='black')  # Crea l'istogramma con assi definiti
plt.xlabel('hours')  # Etichetta dell'asse x
plt.ylabel('APS')  # Etichetta dell'asse y
plt.title('Histogram JSU-DNN method')  # Titolo del grafico
