...
% spreads values considering the accrual term
...
eqn = @(x) spreadsCDS(1)*delta1(1)*discountsCDS(1)*x + spreadsCDS(1)*(delta1(1)/2)*discountsCDS(1)*(1 - x) - LGD*discounts(1)*(1 - x);
survProbs(1) = fsolve(eqn, 1, options);

% Compute the following values with a loop (necessary since each iteration
% uses the values computed in the previous one)
for i=2:length(datesCDS)
    eqn = @(x) spreadsCDS(i)*sum(delta1(1:i).*discountsCDS(1:i).*[survProbs(1:i-1), x]) + spreadsCDS(i)*sum((delta1(1:i)./2).*discountsCDS(1:i).*([1, survProbs(1:i-1)] - [survProbs(1:i-1), x])) - LGD*sum(discountsCDS(1:i).*([1, survProbs(1:i-1)] - [survProbs(1:i-1), x]));
